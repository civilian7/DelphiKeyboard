...
이 키보드는 OS IME 를 사용하지 않고 내장 오토마타로 한글을 조합하므로, 한국어 IME 설치나
시스템 로캘 변경 없이 **어떤 표시 언어의 Windows**(영문·일문 등)에서도 동작합니다.
DLL 인터페이스도 UTF-16(`wchar_t`) 기반이라 시스템 코드페이지와 무관합니다.

### 한국어 폰트 설치

한글 렌더링에는 **맑은 고딕(Malgun Gothic)** 을 사용하며, 이 폰트는 언어와 무관하게 모든
데스크톱 Windows 기본 설치에 포함되어 있어 보통은 설치할 것이 없습니다. Windows Server
나 경량화된 이미지 등에서 한글이 네모(□)로 보이면 아래 방법 중 하나로 설치하세요.

- **설정**: 설정 → 앱 → 선택적 기능 → **기능 추가** → **"한국어 추가 글꼴"(Korean
  Supplemental Fonts)** 설치
- **PowerShell (관리자 권한)**:
...
- **언어 팩**: 설정 → 시간 및 언어 → 언어 및 지역 → **언어 추가** → **한국어**.
  언어 팩과 함께 한국어 폰트가 설치됩니다 (같이 설치되는 한국어 IME 는 이 키보드에
  필요하지 않음)

### 콘솔 출력 참고

비한국어 콘솔 코드페이지에서는 C#/C 콘솔 데모가 출력하는 한글이 `??` 로 보일 수
있습니다. 콘솔 출력을 UTF-8 로 전환하세요 (`chcp 65001`, C# 은
`Console.OutputEncoding = System.Text.Encoding.UTF8`). 이는 콘솔 **표시**에만 해당하며
키보드 입력 자체와는 무관합니다 (Python 3.6+ 는 콘솔 API 직접 출력이라 문제 없음).
...
- **클릭음 유실 수정**: `PlaySound` 는 호출마다 오디오 디바이스를 새로 열고 이전 소리를
  중단시켜 빠른 연타 시 클릭음이 간헐적으로 유실되던 문제를, **waveOut** 디바이스 상시
  오픈(최초 1회) + 헤더 풀 8개 방식으로 전환해 연타 시에도 소리가 겹쳐 재생되도록 수정
- 수정 반영해 DLL 재빌드 (`VKB_Version` 1.4 반환, ABI 변경 없음)

### 2026-07-11 (v1.3)

- **비한국어 Windows 폰트 매칭 수정**: 입력 표시부 폰트를 지역화 이름(`'맑은 고딕'`)으로
  지정하고 있어 비한국어 Windows 에서 매칭에 실패할 수 있던 문제를 영문 정식 이름
  **`'Malgun Gothic'`** 으로 변경 — 어떤 언어의 Windows 에서도 동일하게 렌더링
- **README**: 비한국어 Windows 지원·한국어 폰트 설치 가이드 섹션 추가
- 수정 반영해 DLL 재빌드 (`VKB_Version` 1.3 반환, ABI 변경 없음)
